...
class ReadAcademicYearListScreen extends StatelessWidget {
  const ReadAcademicYearListScreen({super.key});

  @override
  Widget build(BuildContext context) {
    final provider = context.read<DeleteAcademicYearProvider>();

    return Consumer<ReadAcademicYearListProvider>(
      builder: (context, list, _) {
        ...
        return Stack(
          children: [
            AppScreen(
              appBar: AppBar(title: const Text('Read AcademicYear List')),
              children: [
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  spacing: AppSize.medium,
                  children: [
                    AppText("AcademicYear List", variant: AppTextVariant.h2),
                    AppButton(
                      "Create AcademicYear",
                      onPressed: () async {
                    ...
                    AppTableList(
                      ...
                      data: list.academicYears,
                      cellLink: ['name'],
                      onDetail: (id) async {
                        await Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (_) => ReadAcademicYearDetailPage(id),
                          ),
                        );
