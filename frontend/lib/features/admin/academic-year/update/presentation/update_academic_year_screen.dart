...
class UpdateAcademicYearScreen extends StatefulWidget {
  final String id;
  const UpdateAcademicYearScreen(this.id, {super.key});

  @override
  State<UpdateAcademicYearScreen> createState() =>
      _UpdateAcademicYearScreenState();
}

class _UpdateAcademicYearScreenState extends State<UpdateAcademicYearScreen> {
      ...
      if (mounted) {
        ...
        if (detail.academicYear != null) {
          final startDate = DateFormat(
            'dd/MM/yyyy',
          ).format(detail.academicYear!.startDate);

          final endDate = DateFormat(
            'dd/MM/yyyy',
          ).format(detail.academicYear!.endDate);
  ...
  Widget build(BuildContext context) {
    final provider = context.watch<UpdateAcademicYearProvider>();
    final detail = context.watch<ReadAcademicYearDetailProvider>();

    return Stack(
      children: [
        AppScreen(
          appBar: AppBar(title: const Text('Update AcademicYear')),
          children: [
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              spacing: AppSize.xLarge,
              children: [
                AppText("Update AcademicYear", variant: AppTextVariant.h2),
                AppButton(
                  "Save",
                  onPressed: () async {
                    if (!_validate()) return;
                    ...
                    final result = await provider.updateAcademicYear(
                      AcademicYearModel(
                        id: widget.id,
                        name: name,
